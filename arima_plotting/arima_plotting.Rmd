...
```{r}
extract_model_fit(arima_models$arima_model, "aic")

# arima_models %>% 
#   mutate(arima_model_type = extract_model_params(arima_model))
...
calculate_model_fit <- function(model_order, 
  ...
  arima_models %>%
    ...
    extract_model_fit(fit_param)
}


calculate_model_fit(c(0, 1, 0))
calculate_model_fit(c(0, 1, 0), drift = TRUE)

params <- list(a = c(0, 1, 2), b = c(0, 1, 2), c = c(0, 1, 2), drift = c(TRUE, FALSE))
all_params <- do.call(expand_grid, params) %>% 
  filter(!(b == 2 & drift == TRUE))

z <- purrr::pmap(all_params,
            ~ calculate_model_fit(c(..1, ..2, ..3),
                                  drift = ..4)
            )

extract_model_fit(list(z[[3]][[1]]$result), "aic")
