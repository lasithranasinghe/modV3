...
# TODO lots of parsing errors - have these been checked?
world <- read_csv(here::here("data", "TB_notifications.csv"))
# TODO use variable names here rather than numbers if possible
# higher risk of changing data structure and silent miscalculation
...
```{r}
# function to return data frame of sums by WHO region for a given year
group_by_who_region <- function(year) {
  stopifnot(
    is.numeric(year)
  )
        q_year <- enquo(year)
        group_by(world_2013, g_whoregion) %>%
                filter(.data$year == !!q_year) %>%
                summarise(
                        newrel_m04 = sum(na.omit(.data$newrel_m04)),
                        newrel_f04 = sum(na.omit(.data$newrel_f04)),
                        newrel_m514 = sum(na.omit(.data$newrel_m514)),
                        newrel_f514 = sum(na.omit(.data$newrel_f514)),
                        newrel_m014 = sum(na.omit(.data$newrel_m014)),
                        newrel_f014 = sum(na.omit(.data$newrel_f014)),
                        newrel_m15plus = sum(na.omit(.data$newrel_m15plus)),
                        newrel_f15plus = sum(na.omit(.data$newrel_f15plus))
                ) %>%
                as.data.frame() %>%
                mutate(year = !!q_year) %>%
          select(g_whoregion, year, everything())
}

years <- 2013:2020

bind_world <- purrr::map(years,
           group_by_who_region) %>%
...
# TODO might be easier to have totals and percentages in different data frames
mutate_world <-
  mutate(
    bind_world,
    "newrel_tot04" = newrel_m04 + newrel_f04,
    "newrel_tot514" = newrel_m514 + newrel_f514,
    "newrel_tot014" = newrel_m014 + newrel_f014,
    "newrel_tot15plus" = newrel_m15plus + newrel_f15plus
  )

perc_world <-
  mutate(
    mutate_world,
    "perc_014" = 100 * (newrel_tot014 / (newrel_tot014 + newrel_tot15plus)),
    "perc_youngkids" = 100 * (newrel_tot04 / newrel_tot014)
  )
...
create_barplot <- function(df, ylim, ylab = "Notifications", title) {
  barplot(
    df,
...
render_plot_by_who_region <- function(df = perc_world,
                                      region,
                                      age_identifier,
                                      ylim,
                                      plot_title) {
        stopifnot(
                is.data.frame(perc_world),
                is.character(region),
                is.character(age_identifier),
                is.numeric(ylim),
                is.character(plot_title)
        )

        regex_pattern <- paste0("^newrel_[mf]{1}", age_identifier)
        region <- enquo(region)
        final_ylim <- c(0, ylim)

        wide <- df %>%
                filter(g_whoregion == !!region) %>%
                select(matches(regex_pattern)) %>% t()

        colnames(wide) <- c("2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020")

        # TODO why is the final column removed here?
        wide <- wide[c(2,1), -8]

        create_barplot(
          wide, ylim = final_ylim,
          title = plot_title)

}

age_identifiers <- c("04", "514", "014", "15plus")
upper_ylims <- list(30000, 3000, 20000, 3000, 60000, 20000)
titles <- c(
  glue::glue("Sex Disaggregated Notifications for Ages 0-4 Years in {who_regions}")
)
...
p_04 <- purrr::pmap(
  list(who_regions,
       upper_ylims,
       titles),
  .f = function(who_regions, upper_ylims, titles) {
    render_plot_by_who_region(
      region = who_regions,
      age_identifier = age_identifiers[[1]],
      ylim = upper_ylims,
      plot_title = titles
    )
  }) %>% purrr::set_names(who_regions)
...
upper_ylims <- list(40000, 4000, 30000, 5000, 100000, 30000)
titles <- c(
  glue::glue("Sex Disaggregated Notifications for Ages 5-14 Years in {who_regions}")
)
...
p_514 <- purrr::pmap(
  ...
  .f = function(who_regions, upper_ylims, titles) {
    render_plot_by_who_region(
      region = who_regions,
      age_identifier = age_identifiers[[2]],
      ylim = upper_ylims,
      plot_title = titles
    )
  }) %>% purrr::set_names(who_regions)
...
upper_ylims <- list(70000, 6000, 50000 ,8000, 150000 ,50000)
titles <- c(
  glue::glue("Sex Disaggregated Notifications for Ages 0-14 Years in {who_regions}")
)
...
p_014 <- purrr::pmap(
  ...
  .f = function(who_regions, upper_ylims, titles) {
    render_plot_by_who_region(
      region = who_regions,
      age_identifier = age_identifiers[[3]],
      ylim = upper_ylims,
      plot_title = titles
    )
  }) %>% purrr::set_names(who_regions)
...
upper_ylims <- list(1000000, 300000, 300000, 200000, 2000000, 1000000)
titles <- c(
  glue::glue("Sex Disaggregated Notifications for Ages 0-14 Years in {who_regions}")
)
...
p_15 <- purrr::pmap(
  ...
  .f = function(who_regions, upper_ylims, titles) {
    render_plot_by_who_region(
      region = who_regions,
      age_identifier = age_identifiers[[4]],
      ylim = upper_ylims,
      plot_title = titles
    )
  }) %>% purrr::set_names(who_regions)
...
mf_years <- mf_years[c(2,1), c(1:7)]

mf_plot <- create_barplot(mf_years,
                          ylim = c(0, 5000000),
                          title = "Sex Disaggregated Notifications for All Ages Globally")
```
...
sum_afrm04 <- print(summary(fcst_afrm04))


# find all relevent variables
var_name_matches <- str_extract(names(tot_world),
                       pattern = "[mf]{1}[0-9]+.*")

var_groups <- var_name_matches[!is.na(var_name_matches)]

# function to prepare data for arima modelling
prepare_data <- function(df, region, exclude_year = 2020, select_regex) {
        keep_rows <- which(df$year != exclude_year & df$g_whoregion == region)
        filtered <- df[keep_rows,]
        select(filtered, contains(select_regex))
}

# arima model
arima_predict <- function(df) {

        model <- df %>%
                ts(start = 2013, frequency = 1) %>%
                auto.arima(d=1, D=1, stepwise = FALSE, approximation = FALSE, trace = TRUE)
        FCST <- forecast(model, h = 1)
        print(summary(FCST))
}

# list of data frames - estimates for 2020 data by region, age and gender
estimates <- purrr::map(
  who_regions,
  .f = function(region) {
    purrr::map(
      ...
      .f = function(regex) {
        ...
        predict <- arima_predict(df)
        # TODO generally not a good idea to use row names
        # use additional variables instead
        rownames(predict) <-
          paste0(tolower(region), "_", regex)
        predict
      }
    ) %>% reduce(rbind)
  }
) %>% purrr::set_names(tolower(who_regions))

####Isolating time series estimates to populate Seddon tables
#AFR
afr_sed <- cbind(rownames(estimates$afr), estimates$afr)
rownames(afr_sed) <- NULL
colnames(afr_sed) <- c("age_group", "prediction")
afr_sed <- afr_sed[, c(1,2)]
afr_sed <-  pivot_wider(afr_sed, names_from = age_group, values_from = prediction)
afr_sed <- mutate(afr_sed, "all_04" = afr_m04 + afr_f04, "all_514" = afr_m514 + afr_f514, "all_15plus" = afr_m15plus + afr_f15plus)
afr_sed %<>% as.data.frame()
colnames(afr_sed) <- c("m04", "f04", "m514", "f514", "m014", "f014", "m15plus", "f15plus", "all_04", "all_514", "all_15plus")

#AMR
amr_sed <- cbind(rownames(estimates$amr), estimates$amr)
rownames(amr_sed) <- NULL
colnames(amr_sed) <- c("age_group", "prediction")
amr_sed <- amr_sed[, c(1,2)]
amr_sed <-  pivot_wider(amr_sed, names_from = age_group, values_from = prediction)
amr_sed <- mutate(amr_sed, "all_04" = amr_m04 + amr_f04, "all_514" = amr_m514 + amr_f514, "all_15plus" = amr_m15plus + amr_f15plus)
amr_sed %<>% as.data.frame()
colnames(amr_sed) <- c("m04", "f04", "m514", "f514", "m014", "f014", "m15plus", "f15plus", "all_04", "all_514", "all_15plus")

#EMR
emr_sed <- cbind(rownames(estimates$emr), estimates$emr)
rownames(emr_sed) <- NULL
colnames(emr_sed) <- c("age_group", "prediction")
emr_sed <- emr_sed[, c(1,2)]
emr_sed <-  pivot_wider(emr_sed, names_from = age_group, values_from = prediction)
emr_sed <- mutate(emr_sed, "all_04" = emr_m04 + emr_f04, "all_514" = emr_m514 + emr_f514, "all_15plus" = emr_m15plus + emr_f15plus)
emr_sed %<>% as.data.frame()
colnames(emr_sed) <- c("m04", "f04", "m514", "f514", "m014", "f014", "m15plus", "f15plus", "all_04", "all_514", "all_15plus")

#EUR
eur_sed <- cbind(rownames(estimates$eur), estimates$eur)
rownames(eur_sed) <- NULL
colnames(eur_sed) <- c("age_group", "prediction")
eur_sed <- eur_sed[, c(1,2)]
eur_sed <-  pivot_wider(eur_sed, names_from = age_group, values_from = prediction)
eur_sed <- mutate(eur_sed, "all_04" = eur_m04 + eur_f04, "all_514" = eur_m514 + eur_f514, "all_15plus" = eur_m15plus + eur_f15plus)
eur_sed %<>% as.data.frame()
colnames(eur_sed) <- c("m04", "f04", "m514", "f514", "m014", "f014", "m15plus", "f15plus", "all_04", "all_514", "all_15plus")

#SEA
sea_sed <- cbind(rownames(estimates$sea), estimates$sea)
rownames(sea_sed) <- NULL
colnames(sea_sed) <- c("age_group", "prediction")
sea_sed <- sea_sed[, c(1,2)]
sea_sed <-  pivot_wider(sea_sed, names_from = age_group, values_from = prediction)
sea_sed <- mutate(sea_sed, "all_04" = sea_m04 + sea_f04, "all_514" = sea_m514 + sea_f514, "all_15plus" = sea_m15plus + sea_f15plus)
sea_sed %<>% as.data.frame()
colnames(sea_sed) <- c("m04", "f04", "m514", "f514", "m014", "f014", "m15plus", "f15plus", "all_04", "all_514", "all_15plus")

#WPR
wpr_sed <- cbind(rownames(estimates$wpr), estimates$wpr)
rownames(wpr_sed) <- NULL
colnames(wpr_sed) <- c("age_group", "prediction")
wpr_sed <- wpr_sed[, c(1,2)]
wpr_sed <-  pivot_wider(wpr_sed, names_from = age_group, values_from = prediction)
wpr_sed <- mutate(wpr_sed, "all_04" = wpr_m04 + wpr_f04, "all_514" = wpr_m514 + wpr_f514, "all_15plus" = wpr_m15plus + wpr_f15plus)
wpr_sed %<>% as.data.frame()
colnames(wpr_sed) <- c("m04", "f04", "m514", "f514", "m014", "f014", "m15plus", "f15plus", "all_04", "all_514", "all_15plus")

#Merging all
bind_sed <- rbind(afr_sed, amr_sed, emr_sed, eur_sed, sea_sed, wpr_sed)
regions <- c("afr", "amr", "emr", "eur", "sea", "wpr")
bind_sed <- cbind(bind_sed, regions)
bind_sed <- bind_sed[,c(12, 1:11)]

###Finding notification data for Seddon tables
# TODO cannot find grouped_2020 data frame
sed_2020 <- grouped_2020[, -10]
sed_2020 <- mutate(sed_2020, "all_04" = newrel_m04 + newrel_f04, "all_514" = newrel_m514 + newrel_f514, "all_15plus" = newrel_m15plus + newrel_f15plus)
...
est_afr_2020 <- cbind(afr_2020, estimates$afr)
...
est_sea_2020 <- cbind(sea_2020, estimates$sea)
...
est_wpr_2020 <- cbind(wpr_2020, estimates$wpr)
...
est_eur_2020 <- cbind(eur_2020, estimates$eur)
...
est_amr_2020 <- cbind(amr_2020, estimates$amr)
...
est_emr_2020 <- cbind(emr_2020, estimates$emr)
...
arima_copy <- ggplot(piv_copy, aes(x = g_whoregion, y = value, fill = sex)) + facet_wrap(~piv_copy$age_group) + geom_bar(stat = "identity", position = "dodge") + xlab("Region") + ylab("Percentage Difference") + labs(title = "Percentage Difference Between Actual Notifications and Time Series Prediction by Region") + theme(plot.title = element_text(size = 9)) + scale_fill_discrete(name = "Sex") + scale_fill_manual(labels = c("Female", "Male"), values = c("seagreen1", "skyblue")) + theme(axis.text.x = element_text(size = 7))
arima_copy
...
males_04 <- select(perc_world, c(g_whoregion, year, newrel_m04))
males_04 <-
  mutate(males_04, newrel_m04 = ifelse(year == "2020", NA, newrel_m04))
gg_males04 <-
  ggplot(males_04, aes(x = year, y = newrel_m04, col = g_whoregion)) + geom_line() + geom_point(x = 2020, y = 27043, col = "red") + geom_point(x = 2020, y = 2062, col = "brown") + geom_point(x = 2020, y = 17243.17, col = "green") + geom_point(x = 2020, y = 1465, col = "turquoise2") + geom_point(x = 2020, y = 58951.83, col = "blue") + geom_point(x = 2020, y = 12558, col = "pink") + scale_y_continuous(limits = c(0, 60000)) + labs(title = "Notifications Time Series for Males 0-4 yrs")
...
#create all tables
test_males04 + xlab("Year") + ylab("Notifications") + labs(title = "Trend in Notifications from 2013 to 2020 for Males Aged 0-4 Years", subtitle = "Including Actual and Predicted Notifications for 2020") + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5)) + scale_color_discrete(name = "Region")

test_females04 + xlab("Year") + ylab("Notifications") + labs(title = "Trend in Notifications from 2013 to 2020 for Females Aged 0-4 Years", subtitle = "Including Actual and Predicted Notifications for 2020") + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5)) + scale_color_discrete(name = "Region")

test_males514 + xlab("Year") + ylab("Notifications") + labs(title = "Trend in Notifications from 2013 to 2020 for Males Aged 5-14 Years", subtitle = "Including Actual and Predicted Notifications for 2020") + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5)) + scale_color_discrete(name = "Region")

test_females514 + xlab("Year") + ylab("Notifications") + labs(title = "Trend in Notifications from 2013 to 2020 for Females Aged 5-14 Years", subtitle = "Including Actual and Predicted Notifications for 2020") + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5)) + scale_color_discrete(name = "Region")

test_males15plus + xlab("Year") + ylab("Notifications") + labs(title = "Trend in Notifications from 2013 to 2020 for Males Aged Over 15 Years", subtitle = "Including Actual and Predicted Notifications for 2020") + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5)) + scale_color_discrete(name = "Region")

test_females15plus + xlab("Year") + ylab("Notifications") + labs(title = "Trend in Notifications from 2013 to 2020 for Females Aged Over 15 Years", subtitle = "Including Actual and Predicted Notifications for 2020") + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5)) + scale_color_discrete(name = "Region")
...
all_copy04$year <- as.numeric(all_copy04$year)
...
###Drawing the plots
world_04 <- ggplot(all_copy04, aes(x = factor(year), y = cases, fill = sex)) + geom_bar(stat = "identity", position = "dodge") + labs(title = "Number of Reported Cases Amongst 0-4 yrs from 2013 to 2020") + xlab("Year") + ylab("Notifications") + scale_fill_manual(name = "Sex", labels = c("Female", "Male"), values = c("seagreen1", "skyblue")) + theme(axis.text.x = element_text(size = 10))


world_514 <- ggplot(all_copy514, aes(x = factor(year), y = cases, fill = sex)) + geom_bar(stat = "identity", position = "dodge") + labs(title = "Number of Reported Cases Amongst 5-14 yrs from 2013 to 2020") + xlab("Year") + ylab("Notifications") + scale_fill_manual(name = "Sex", labels = c("Female", "Male"), values = c("seagreen1", "skyblue")) + theme(axis.text.x = element_text(size = 10))

world_15plus <- ggplot(all_copy15plus, aes(x = factor(year), y = cases, fill = sex)) + geom_bar(stat = "identity", position = "dodge") + labs(title = "Number of Reported Cases Amongst Over 15s from 2013 to 2020") + xlab("Year") + ylab("Notifications") + scale_fill_manual(name = "Sex", labels = c("Female", "Male"), values = c("seagreen1", "skyblue")) + theme(axis.text.x = element_text(size = 10))

world_all <- ggplot(all_copyall, aes(x = year, y = cases, fill = sex)) + geom_bar(stat = "identity", position = "dodge") + labs(title = "Number of Reported Cases Amongst All Ages from 2013 to 2020") + xlab("Year") + ylab("Notifications") + scale_fill_manual(name = "Sex", labels = c("Female", "Male"), values = c("seagreen1", "skyblue")) + theme(axis.text.x = element_text(size = 10))
...
piv_four$age_group[piv_four$age_group == "04"] <- "0-4"
piv_four$age_group[piv_four$age_group == "514"] <- "5-14"
piv_four$age_group[piv_four$age_group == "15plus"] <- "15+"


piv_four$age_group <- factor(piv_four$age_group, levels = c("0-4", "5-14", "15+"))
arima_four <- ggplot(piv_four, aes(x = g_whoregion, y = value, fill = sex)) + facet_wrap(~piv_four$age_group) + geom_bar(stat = "identity", position = "dodge") + theme(axis.text.x = element_text(angle = 90)) + xlab("Region") + ylab("Percentage Difference") + labs(title = "Percentage Difference Between Actual Notifications and Time Series Prediction by Region") + theme(plot.title = element_text(size = 9)) + scale_fill_manual(name = "Sex", labels = c("Female", "Male"), values = c("seagreen1", "skyblue")) + theme(axis.text.x = element_text(size = 7)) + theme(plot.title = element_text(hjust = 0.5))
