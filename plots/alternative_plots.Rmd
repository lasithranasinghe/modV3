...
> Sex disaggregated notifications by age group and WHO region

```{r age-region-plot, fig.height = 15, fig.width= 10}
age_region_plots <- dd$long$region %>% 
  filter(!age_group %in% "014",
         year >= 2014) %>% 
  mutate(age_group_cat = factor(age_group, 
                                      ordered = TRUE,
                                  levels = c("04", "514", "15plus"))) %>% 
  ...
  nest() %>% 
  mutate(plots = map2(data, g_whoregion, function(region, name) {
    ggplot(data = region,
           aes(
             x = year,
             y = cases,
      ...
      facet_wrap(. ~ age_group_cat, scales = "free_y") +
      theme_minimal() +
      labs(x = "",
           title = name) +
...
aggregate_plot <- wrap_plots(age_region_plots$plots, nrow = 6) +
  plot_annotation(title = "Notifications between 2013 and 2020 by age group and WHO Region",
                  caption = "Source: WHO Global TB Programme") +
  scale_color_discrete() + 
  theme(legend.position = "bottom")

ggsave(here::here("figures/supplement/age_region_sex.png"), 
       plot = aggregate_plot,
       width = 10,
       height = 15)
