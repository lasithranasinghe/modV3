...
## --- barplots for 30 HBC
## now using estimated shortfall rather than notification change 
...
mpdref[age=='04',age_group:='0-4 years']
mpdref[age=='514',age_group:='5-14 years']
mpdref[age=='15plus',age_group:='15+ years']

mpdh30[,unique(age_group)]
mpdref[,unique(age_group)]

## merge
mpdh30 <- merge(mpdh30,mpdref,by=c('iso3','age_group'))

## total
tot30 <- mpdh30[,.(mid=sum(mid),mid.sd=ssum(mid.sd),
                   cases=sum(cases)),by=age_group]
tot30[,c('lo','hi','iso3','country'):=.(mid-1.96*mid.sd,
                                        mid+1.96*mid.sd,
                                        'HBC30','HBC30')]
...
## ranking
cnz <- mpdh30[age_group!='15+ years',
              .(dfr=sum(cases-mid)),
              by=country][order(dfr),country] #countries in increasing order of estimated total 014 drop
...
mpdh30$age_group<- factor(mpdh30$age_group,
...
GP <- GP +  geom_segment(data = arrowdf,
                         ...
                         colour = "blue", size = 0.5, alpha=0.9,
                         arrow = arrow())
...
## CF <- merge(CF,
##             mpd[age_group == "014", .(iso3, percent_change)],
##             by = "iso3")
...
S <- S[, .(index = mean(value)),
       by = .(iso3 = country_code)] # mean for 2020
...
GA <- ggarrange(plotlist = PL,labels = agz2,ncol=1,vjust=1,hjust=-1)
ggsave(GA,filename=here('plots/stingall.pdf'),w=7,h=12)
ggsave(GA,filename=here('plots/stingall.png'),w=7,h=12)
ggsave(GA,filename=here('plots/stingall.eps'),w=7,h=12)
